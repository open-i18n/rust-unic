...
pub fn read_derived_core_properties() -> io::Result<DerivedCoreProperties> {
    let mut file = File::open(Path::new("data/ucd/DerivedCoreProperties.txt"))?;
    let mut buffer = String::new();
    file.read_to_string(&mut buffer)?;
    Ok(
        buffer
            .parse()
            .expect("Failed to parse DerivedCoreProperties"),
    )
}
